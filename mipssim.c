...
void execute()
{
    ...
    switch (control->PCSource) {
        ...
        case 1:
            ...
            break;
        case 2:
            offset = arch_state.IR_meta.jmp_offset;
            first_bits_pc = (get_piece_of_a_word(curr_pipe_regs->pc, 28, 4));
            next_pipe_regs->pc = offset << 2 | (first_bits_pc >> 28) << 28;
            break; 
        default:
            assert(false);
...
void memory_access() {
    ...
    int data_B = arch_state.curr_pipe_regs.B;
    int address;
    ...
    if(arch_state.control.MemWrite && arch_state.control.IorD == 1){
        memory_write(arch_state.curr_pipe_regs.ALUOut, data_B);
    }
...
void write_back()
{
    int write_reg_id, write_data;
    if (arch_state.control.RegWrite == 1){
...
void set_up_IR_meta(int IR, struct instr_meta *IR_meta)
{
    ...
    switch (IR_meta->opcode) {
        case SPECIAL:
            ...
            printf("Executing SLT(%d), if $%u < $%u $%u = 1 \n",
                IR_meta->opcode, IR_meta->reg_21_25,  IR_meta->reg_16_20, IR_meta->reg_11_15);
            else assert(false);
        ...
        case J:
            ...
            break; 
        case BEQ:
            printf("Executing BEQ(%d), if $%u == $%u advance_pc (%u << 2)\n",
